...
from tabsdatasdk.exceptions import ErrorCode, RegistrationError
...
PYTHON_CHECK_MODULE_AVAILABILITY_KEY = "checkModuleAvailability"
PYTHON_LOCAL_PACKAGES_KEY = "pythonLocalPackages"
PYTHON_REQUIREMENTS_INSTALL_DEPENDENCIES_KEY = "installDependencies"
PYTHON_REQUIREMENTS_KEY = "pythonRequirements"
PYTHON_VERSION_KEY = "pythonVersion"
REQUIREMENTS_FILE_NAME = "requirements.yaml"


def create_configuration(function: DatasetFunction, save_location: str):
...
def create_output_configuration(function: DatasetFunction, save_location: str) -> dict:
...
def convert_to_dict_and_store_if_plugin(
    to_convert: InputPlugin | OutputPlugin | Input | Output, save_location: str
) -> dict:
    configuration_dict = to_convert.to_dict() if to_convert else {}
    if isinstance(to_convert, InputPlugin) or isinstance(to_convert, OutputPlugin):
...
def create_input_configuration(function: DatasetFunction, save_location: str) -> dict:
...
def create_requirements(
    ...
    data = {
        ...
        PYTHON_REQUIREMENTS_KEY: requirements,
        PYTHON_REQUIREMENTS_INSTALL_DEPENDENCIES_KEY: False,
...
def copy_and_verify_requirements_file(
    ...
    try:
        requirements = sorted(
            list(set(data[PYTHON_REQUIREMENTS_KEY])), key=str.casefold
        )
    except KeyError:
        raise RegistrationError(
            ErrorCode.RE9, PYTHON_REQUIREMENTS_KEY, requirements_file, data
        )
    except TypeError:
        raise RegistrationError(
            ErrorCode.RE10,
            ...
            requirements_file,
            type(data[PYTHON_REQUIREMENTS_KEY]),
        )
...
def create_bundle_archive(
    function: DatasetFunction | Callable,
    ...
    Args:
        function (DatasetFunction): The function to be registered.
    ...
    """
    if not isinstance(function, DatasetFunction):
